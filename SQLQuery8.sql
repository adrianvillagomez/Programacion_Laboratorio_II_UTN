...
select * from ventas inner join productos on ventas.id_producto = productos.id
...
select top(3) * from ventas order by id asc
--insert select update delete//inner para teorico
select nombre from usuario where nombre like '%a%'
select nombre from usuario where nombre like '_a%'
select nombre from usuario where nombre like '_a_____'
